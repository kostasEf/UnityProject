...
public class StrelaAnimation : MonoBehaviour
{
    ...
    private StrelaSight strelaSight;          // Reference to the StrelaSight script.
    ...
    void Awake ()
    {
        ...
        strelaSight = GetComponent<StrelaSight>();
    ...
    void NavAnimSetup ()
    {
        ...
        if(strelaSight.enemyInSight)
        {
            // ... the enemy should stop...
            speed = 0f;
            
            // ... and the angle to turn through is towards the enemy.
            angle = FindAngle(transform.forward, enemy.position - transform.position, transform.up);
        }
        else
        {
